...
class freq_env(object):
    ...
    def waterfall_reset(self,sess):
        ...
        rand_1 = np.random.randint(-1, 3)
        sweep[:, 86 + rand_1:105 + rand_1] = 2
        rand_2 = np.random.randint(-1, 3)
        sweep[:, 277 + rand_2:297 + rand_2] = 2
        rand_3 = np.random.randint(-1, 3)
        sweep[:, 395 + rand_3:408 + rand_3] = 2
        rand_4 = np.random.randint(-1, 3)
        sweep[:, 611 + rand_4:638 + rand_4] = 2
        rand_5 = np.random.randint(-1, 3)
        sweep[:, 944 + rand_5:952 + rand_5] = 2
        rand_6 = np.random.randint(-1, 3)
        sweep[:, 961 + rand_6:990 + rand_6] = 2
        rand_7 = np.random.randint(-1, 3)
        sweep[:, 1000 + rand_7:1009 + rand_7] = 2
        rand_8 = np.random.randint(-1, 3)
        sweep[:, 1046 + rand_8:1065 + rand_8] = 2
        rand_9 = np.random.randint(-1, 3)
        sweep[:, 1157 + rand_9:1180 + rand_9] = 2
        rand_10 = np.random.randint(-1, 3)
        sweep[:, 1195 + rand_10:1205 + rand_10] = 2

        self.current_line = -1
        self.user_pos = self.user_action[np.random.randint(0,2)]
        for i in range(self.lines):
            ...
            #sweep[i, self.action_space[17] - 5 - randint:self.action_space[17] + 6 + randint] += 1
            sweep[i, self.action_space[17] - 7:self.action_space[17] + 8] += 1
            self.current_line = (self.current_line + 1) % self.interval
            #self.jamm_pos = self.action_space[(i//self.interval)%self.n_actions]
            self.jamm_pos = self.action_space[17]
        # user
        #self.action_last = np.random.randint(0,19)
        #self.user_pos = 80 * (self.action_last+1)
        #randInt_init = np.random.randint(0,10)
        #sweep[-1, self.user_pos - 5 - randInt_init:self.user_pos + 6 + randInt_init] += 2
        ...
        #waterfall_reset = (waterfall_reset - np.min(waterfall_reset)) / (np.max(waterfall_reset) - np.min(waterfall_reset))
    ...
    def waterfall_next(self,waterfall_reset,waterfall_label,action,sess):
        ...
        for i in range(self.lines):
            if i < self.lines -1:
                ...
                randInt = np.random.randint(0,4)
                randInt_1 = np.random.randint(0, 12)
                #if self.current_line == 9:
                    #self.user_pos_2 = self.user_pos
                print(self.jamm_pos, self.user_pos)
                if self.current_line == self.interval-1:
                    if self.user_pos != 0:
                        self.jamm_pos = self.user_pos
                        #waterfall_n[i,self.jamm_pos - 5 - randInt:self.jamm_pos + 6 + randInt] += 1
                        waterfall_n[i, self.jamm_pos - 7:self.jamm_pos + 8] += 1
                    else:
                        waterfall_n[i, self.jamm_pos - 7:self.jamm_pos + 8] += 1
                else:
                    waterfall_n[i,self.jamm_pos - 7:self.jamm_pos + 8] += 1
                ...
                #user
                #waterfall_n[i,80*(action+1) - 8 - randInt_1:80*(action+1) + 9 + randInt_1] += 2

                #waterfall_n[i, 80 * (action + 1) - 15:80 * (action + 1) + 16] += 2
                #self.user_pos = 80 * (action + 1)
                waterfall_n[i, self.user_action[action] - 15:self.user_action[action] + 16] += 2
                self.user_pos = self.user_action[action]
        ...
        #waterfall_next[-1,:] = (waterfall_next[-1,:]-np.min(waterfall_next[-1,:]))/(np.max(waterfall_next[-1,:])-np.min(waterfall_next[-1,:]))
    ...
    def step(self,waterfall_reset,observation,action,sess):
        ...
        #if (action+1)*80 == self.jamm_pos or action in [0,4,7,11,12,14]:
        if self.user_action[action] == self.jamm_pos:
            reward = -1
            done = True
        ...
        #print("jamm:", self.jamm_pos, "user:", action,(action+1)*80,"done:",done)
        return s_,s_label, reward, done
"""
if __name__ == "__main__":
    env=freq_env()
    saver = tf.train.Saver()
    checkpoint = tf.train.latest_checkpoint("my_train")
    with tf.Session() as sess:
        ...
        #waterfall_next,waterfall_n = env.waterfall_next(waterfall_reset,waterfall_label,9,sess)

        #plt.imshow(waterfall_reset,interpolation='nearest',aspect='auto')
        #plt.show()
