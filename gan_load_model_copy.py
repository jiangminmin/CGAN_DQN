...
class generator():
    def __init__(self):
        self.EPS = 1e-12
        self.CROP_SIZE = 256
        self.Model = collections.namedtuple("Model","outputs, predict_real, predict_fake, discrim_loss, discrim_grads_and_vars, gen_loss_GAN, gen_loss_L1, gen_grads_and_vars, train")
    ...
    def create_model(self,inputs, targets):
        ...
        with tf.variable_scope("generator"):
            out_channels = int(targets.shape[-1])
    ...
    def create_sweep(self,):
        ...
        for i in range(self.lines):
            ...
            sweep[i,self.action_space[self.row%self.n_actions]-10-randint:self.action_space[self.row%self.n_actions]+11+randint]=1#+np.random.normal(0,0.2,1)[0]
    ...
    def generate_img(self,img):
        ...
        #hah = cv2.imread('my_train_img/train0.jpg',0)
        #plt.imshow(hah[:, :1601,], interpolation='nearest', aspect='auto')
        #plt.show()
        #plt.imshow(hah[:, 1601:,], interpolation='nearest', aspect='auto')
        #plt.show()
        #my_img = img
        #my_img = my_img[:,:,tf.newaxis]
        my_img = tf.image.decode_jpeg(tf.read_file('my_train_img/train0.jpg'))
        ...
        #print(tf.Session().run(inputs),inputs.shape)
        plt.imshow(tf.Session().run(inputs)[:, :,0], interpolation='nearest', aspect='auto')
        plt.show()
        targets=tf.image.resize_images(targets, [256, 256], method=tf.image.ResizeMethod.AREA)
        plt.imshow(tf.Session().run(targets)[:, :, 0], interpolation='nearest', aspect='auto')
        plt.show()
        inputs,targets = tf.train.batch([inputs,targets],batch_size=1)


        model = self.create_model(inputs,targets)
        ...
        with sv.managed_session() as sess:
            ...
            img = sess.run(converted_outputs)
        plt.imshow(img[0,:,:,0], interpolation='nearest', aspect='auto')
        plt.show()
