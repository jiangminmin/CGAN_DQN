...
class freq_env(object):
    ...
    def waterfall_reset(self):
        ...
        rand_int_2 = random.randint(0, self.line_num-1)
        pow_ls = []
        for i in range(self.n_actions):
            pow = np.sum(waterfall_reset[0, -1, (i+1) * 80-40:(i+1) * 80+41, 0])
            pow_ls.append(pow)
        min_index = pow_ls.index(min(pow_ls))
        self.action_last = rand_int*500000+100500000
    ...
    def waterfall_next(self, waterfall_last, action):
        ...
        jamm_ls.append(int(self.jammer_obj.get_center_freq()-100500000)/500000)
        """
        for i in range(self.n_actions):
    ...
    def step(self, action, observation):
        ...
        jamm_ls=np.array(jamm_ls)
        print(jamm_ls)
        if len(jamm_ls[np.where((jamm_ls[np.where((100500000+jamm_ls*500000-6250*20)<action)]*500000+6250*20+100500000)>action)])>0:
            reward = -1
            done = True
