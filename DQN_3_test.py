...
ENV_NAME = 'jamm_combo_model'#'Breakout-v0'  # Environment name
FRAME_WIDTH = 50  # Resized frame width
FRAME_HEIGHT = 160  # Resized frame height
        ...
        if env.mode == 0:
            ...
            for _ in range(NUM_EPISODES):
                ...
                while not terminal:
                    last_observation = observation
                    action = agent.get_action(state)
                    observation, reward, terminal= env.step_combo(action,last_observation)
                    #waterfall_figure.imshow(observation[0, :, :, 0])
                    #canvas.draw()
                    processed_observation = preprocess(observation)
                    #print(observation==last_observation)
            ...
            for _ in range(NUM_EPISODES):
                ...
                while not terminal:
                    last_observation = observation
                    action = agent.get_action(state)
                    observation, reward, terminal= env.step_dynamic(action,last_observation)
                    #waterfall_figure.imshow(observation[0, :, :, 0])
                    #canvas.draw()
                    processed_observation = preprocess(observation)
                    #print(observation==last_observation)
            ...
            for _ in range(NUM_EPISODES):
                ...
                while not terminal:
                    last_observation = observation
                    action = agent.get_action(state)
                    observation, reward, terminal= env.step_intelligent(action,last_observation)
                    #waterfall_figure.imshow(observation[0, :, :, 0])
                    #canvas.draw()
                    processed_observation = preprocess(observation)
                    #print(observation==last_observation)
