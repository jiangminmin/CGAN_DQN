...
import usrp_spectrum_sense_copy as usrp_spetrum_sense
...
class freq_env(object):
    ...
    def waterfall_reset(self,):
        ...
        print(self.jamm_pos)
        #waterfall_reset = (waterfall_reset - np.min(waterfall_reset)) / (
                    #np.max(waterfall_reset) - np.min(waterfall_reset)) * 255
        #for i in range(100):
            #waterfall_reset[i,:] = (waterfall_reset[i,:] - np.min(waterfall_reset[i,:])) / (np.max(waterfall_reset[i,:]) - np.min(waterfall_reset[i,:])) * 255
    ...
    def waterfall_next(self,waterfall_last,action):
        ...
        #waterfall_next[-1,:] = (waterfall_next[-1,:] - np.min(waterfall_next[-1,:])) / (np.max(waterfall_next[-1,:]) - np.min(waterfall_next[-1,:])) * 255
        self.jamm_pos = (int(self.jammer_obj.get_center_freq()) - 100000000)/6250
        print(self.jamm_pos)
        print(waterfall_next[0,:])
